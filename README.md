<div align="center">
  ...
  <p>
    ...
    <a href="https://hub.flec.top">使用文档</a> /
...
- `flecb install` - 交互式安装 FlecBlog
- `flecb upgrade` - 升级到最新版本
- `flecb reconfig` - 修改配置（端口、API地址等）
- `flecb backup` - 备份和恢复数据
- `flecb uninstall` - 卸载 FlecBlog
- `flecb version` - 查看版本信息

**安装流程：**

1. 运行安装命令 `curl -fsSL https://install.flec.top | sudo bash`
2. 选择部署方式（Docker 部署）
3. 配置端口（Server、Blog、Admin）
4. 确认配置后自动拉取镜像并启动服务
5. 访问管理端创建管理员账户
