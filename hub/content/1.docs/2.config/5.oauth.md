---
title: OAuth 配置
description: FlecBlog 支持通过第三方账号快速登录，基于 Goth 库实现，目前支持 GitHub、Google、QQ、Microsoft 等主流平台。
navigation:
...
1. 我们首先需要访问 [Google API Console](https://console.developers.google.com/) 获取 OAuth 2.0 凭据，点击左侧边栏的 *凭证* 选项
2. 创建一个 *OAuth 客户端 ID*
   - **应用类型：** Web 应用
   - **名称：** 应用名称，建议使用博客名称
...
1. 进入 [QQ 互联](https://connect.qq.com/) 创建应用
...
3. 填写站点信息：
   ...
   - **网站地址备案号：**&#x56FD;内服务器备案号，若没有可联系客服解决，上同
     - [QQ互联WIKI](https://wiki.connect.qq.com/%e5%bc%80%e5%8f%91%e8%80%85%e5%8f%8d%e9%a6%88) 在此处找客服，建议选择腾讯开放平台网站客服，响应还是比较快的
   - **网站图标：** 100\*100的站点图标，用于品牌塑造
4. 创建完成后就可以看到 `APP ID` 和 `APP Key`
...
2. 在 Azure 服务里找到 *Microsoft Entra ID* 进入，在左侧边栏找到*应用注册*，添加新注册
3. 注册应用程序
   - **名称：** 应用名称，建议使用博客名称
...
6. 在左侧边栏找到*证书和密码*，新增客户端密码，期限最多 730 天，创建后表格中的值列，即 **Client Secret**
7. (可选)还可以在左侧边栏的 *品牌打造和属性* 里，设置应用信息来获得更专业的形象

## OIDC 登录

1. 选择合适的 OIDC 身份提供商并进入，比如 [Logto](https://logto.io/)
2. 创建一个租户或者选择已有租户，找到 *Applications* 进入，创建一个 *Traditional Web* 应用
3. 填写站点信息：
   - **Application name：** 应用名称，建议使用博客名称
   - **Redirect URIs：** 回调地址 `https://后端域名/api/v1/auth/oidc/callback`
4. 然后在下面获取 `Issuer endpoint`（即 Issuer URL）、 `App ID` 和 `App secrets`
